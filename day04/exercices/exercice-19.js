...
let nombres = [10, 20, 30, 40, 45, 50, 60]
function rechercheBinaire(tab, num) {
    let debut = 0
    let fin = tab.length - 1

    while (debut <= fin) {
        let m = Math.floor((debut + fin) / 2)

        if (tab[m] === num) return m
...
let result = rechercheBinaire(nombres, 5)
console.log(result === 0 ? "Élément non trouvé" : result)
